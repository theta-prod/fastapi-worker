...
def getModel(tag: str, num_labels: int = 2) -> BertForTokenClassification:
...
model: BertForTokenClassification = getModel("theta/deeper")
classifier = pipeline("text-classification", model, tokenizer=tokenizer, return_all_scores=True)



def runModel(data: str) -> List[ModelResult]:
    labelMaping: Dict[str,str] = {
        "LABEL_0": "低強度",
        "LABEL_1": "高強度"
    }
    return [{
        "label": labelMaping[i['label']],
        "score": i['score']
    } for i in classifier(data)[0]]
