...
def getModel(tag: str):
    return AutoModelWithLMHead.from_pretrained(tag)



##
model = getModel("theta/gpt2-reporter")
classifier = pipeline('text-generation',model=model, tokenizer='uer/gpt2-chinese-cluecorpussmall')


def runModel(data: str, max_length: int =150) -> str:
    modelResult = classifier(data, max_length=max_length)
    return modelResult[0]['generated_text']
